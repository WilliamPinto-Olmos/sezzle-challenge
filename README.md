...
- The browser calls the relative URL `/calculations`. Vite forwards it to `http://localhost:8080`, keeping development requests on the same origin without adding CORS configuration. `API_PROXY_TARGET` overrides the destination; Compose uses `http://backend:8080` so requests reach the backend container. A deployed static build needs its host to forward `/calculations` and serve `index.html` for frontend paths; Vite's development proxy is not included in the build.
- The layout is mobile-first, with named buttons, visible focus, live result/error announcements, and reduced-motion support. Keyboard handling respects native button activation, editable fields, and separate calculator instances.
